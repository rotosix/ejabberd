...
call(Client = #tclient{}, Function, Args)
  ...
  %%error_logger:info_msg("client Function ~p, Args ~p, Client ~p", [Function, Args, Client]),
    case send_function_call(Client, Function, Args) of
        {Client1, ok} ->
          %%error_logger:info_msg("client Function ~p, Args ~p, Client1 ~p", [Function, Args, Client1]),
...
%% begin modify by james.wu@asiainnovations 2015-01-29
%%                                            seqid = SeqId},
                                            seqid = SeqId+1},
%% end modify by james.wu@asiainnovations 2015-01-29
...
%%            {Client#tclient{protocol = Proto4}, ok}
            {Client#tclient{protocol = Proto4, seqid=SeqId+1}, ok}
%% end modify by james.wu@asiainnovations 2015-01-29
    end.
         ...
         {Proto1, MessageBegin} ->
             ...
             case MessageBegin of
                 #protocol_message_begin{seqid = RetSeqId} when RetSeqId =/= SeqId ->
                     %% james.wu {NewClient, {error, {bad_seq_id, SeqId}}};
                     {NewClient, {error, {bad_seq_id, SeqId, RetSeqId, Client, Function}}};
