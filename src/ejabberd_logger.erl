...
start() ->
    ...
    application:set_env(
      ...
      %% begin modify by james.wu@asiainnovations.com on 2014-12-24 PM 3:47
      %%{lager_file_backend, [{file, ConsoleLog}, {level, info}, {size, 0}, {count, 1}, {async_threshold, 20000}, {async_threshold_window, 5000}]},
      %%{lager_file_backend, [{file, ErrorLog}, {level, error}, {size, 0}, {count, 1}, {async_threshold, 20000}, {async_threshold_window, 5000}]}]),
      {lager_file_backend, [{file, ConsoleLog}, {level, info}, {size, 0}, {count, 1}]},
      {lager_file_backend, [{file, ErrorLog}, {level, error}, {size, 0}, {count, 1}]}]),
      %% end modify by james.wu@asiainnovations.com on 2014-12-24 PM 3:47
