...
%% modify by james.wu@asiainnovations.com 2014-09-04 AM 11:12
do_route(From, To, Packet) ->
    ?DEBUG("local route~n\tfrom ~p~n\tto ~p~n\tpacket "
	   "~P~n",
	   [From, To, Packet, 8]),
    if To#jid.luser =/= <<"">> , To#jid.luser =/= From#jid.luser ->
	    ejabberd_sm:route(From, To, Packet);
	  From#jid.luser =:= To#jid.luser , From#jid.lresource =:= <<"">> ->
      ?DEBUG("local route~n\tfrom ~p~n\tto ~p~n\tlresource ~p~n", [From#jid.luser, To#jid.luser, From#jid.lresource]),
	    ejabberd_sm:route(From, To, Packet);
    To#jid.lresource =:= <<"">> ->
	    #xmlel{name = Name} = Packet,
       ?DEBUG("local route~n\tto ~p~n\tname ~p~n", [To#jid.lresource, Name]),
	   case Name of
