		...
		case ejabberd_auth:check_password_with_authmodule(U,
		  ...
		  {true, AuthModule} ->
			...
			Info = [{ip, StateData#state.ip}, {conn, Conn},
			        %% begin add by james.wu@asiainnovations 2015-03-12
				    %%{auth_module, AuthModule}],
				    {auth_module, AuthModule},
				    {sockmod, StateData#state.sockmod}, {socket, StateData#state.socket}],
				    %% end add by james.wu@asiainnovations 2015-03-12
...
wait_for_feature_request({xmlstreamelement, El},
    ...
    case {xml:get_attr_s(<<"xmlns">>, Attrs), Name} of
	  ...
	  if (SockMod == gen_tcp) and TLSRequired ->
         ...
         %% modify by james.wu@asiainnovations.com for xmlns = <<"jabberd:iq:auth">>
         %% {xmlel,<<"presence">>,[{<<"id">>,<<"7zOOM-89">>},{<<"type">>,<<"unavailable">>}],[]}
         case El of
           #xmlel{children = [Query | _]} = El ->
               case {xml:get_tag_attr_s(<<"xmlns">>, Query), Name} of
                   {?NS_AUTH, <<"iq">>} ->
                       #xmlel{children = [Username | _]} = Query,
                       Uid = xml:get_tag_attr_s(<<"username">>, Username),
                       MsgId = xml:get_tag_attr_s(<<"id">>, El),
                       ?INFO_MSG("~n~p ~p, ~p, ~p, ~p", [?NS_AUTH, Query, Name, Uid, MsgId]),
                       Res = #iq{type = result, id = MsgId,
                           sub_el =
                               [#xmlel{name = <<"query">>,
                                   attrs = [{<<"xmlns">>, ?NS_AUTH}],
                                   children =
                                       [#xmlel{name = <<"username">>, attrs = [], children = [{xmlcdata, Uid}]},
                                       #xmlel{name = <<"password">>, attrs = [], children = []},
                                       #xmlel{name = <<"resource">>, attrs = [], children = []}]}]},
...
resource_conflict_action(U, S, R) ->
  ...
  ?DEBUG("U:~p, S:~p, R:~p, OptionRaw:~p~n", [U, S, R, OptionRaw]),
	...
	#iq{type = set, xmlns = ?NS_SESSION} ->
		...
		allow ->
		    ...
		    Info = [{ip, NewState#state.ip}, {conn, Conn},
			    ...
			    {auth_module, NewState#state.auth_module},
			    {sockmod, NewState#state.sockmod}, {socket, NewState#state.socket}],
			    %% end add by james.wu@asiainnovations 2015-03-12
    ...
    case StateData#state.mgmt_state of
	  ...
	  if StateName == session_established;
		 ...
		 case StateData#state.authenticated of
		   ...
		   _ ->
         ?INFO_MSG("Close session for Reason ~p StateName ~p StateData ~p",
...
update_priority(Priority, Packet, StateData) ->
    Info = [{ip, StateData#state.ip}, {conn, StateData#state.conn},
        %% begin add by james.wu@asiainnovations 2015-03-13
	    %%{auth_module, StateData#state.auth_module}],
	    {auth_module, StateData#state.auth_module},
	    {sockmod, StateData#state.sockmod}, {socket, StateData#state.socket}],
	    %% end add by james.wu@asiainnovations 2015-03-13
